...
const EventForm = ({ userId, type, event, eventId }: EventFormProps) => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // cek apakah image sudah diupload atau belum
    if(files.length > 0) {
    ...
    if(type === 'Update') {
      ...
      try {
        const updatedEvent = await updateEvent({
          ...
          path: `/events/${eventId}` // set redirect path after update
        })

        if(updatedEvent) {
          form.reset();
          router.push(`/events/${updatedEvent._id}`) // redirect path after update
        }
      } catch (error) {
        console.log(error);
      }
